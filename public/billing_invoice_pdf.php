...
$tmpBase = tempnam(sys_get_temp_dir(), 'package_receipt_');
if ($tmpBase === false) {
    http_response_code(500);
    echo 'Could not prepare receipt PDF.';
    exit;
}

$pdfFile = dirname($tmpBase) . DIRECTORY_SEPARATOR . uniqid('package_receipt_', true) . '-' . $service->buildFilename($invoice);
@unlink($tmpBase);

$service->renderPdfToFile($invoice, $pdfFile);
header('Content-Type: application/pdf');
header('Content-Disposition: inline; filename="' . $service->buildFilename($invoice) . '"');
readfile($pdfFile);
@unlink($pdfFile);
exit;
