...
$formScheduleName = (string) ($_POST['schedule_name'] ?? '');
$formReportId = (int) ($_POST['report_id'] ?? 0);
$formScheduleType = (string) ($_POST['schedule_type'] ?? 'daily');
$formFormat = (string) ($_POST['format'] ?? 'csv');
$formRecipients = array_values(array_filter(array_map('trim', (array) ($_POST['recipients'] ?? ['']))));
$formIsActive = !isset($_POST['is_active']) || $_POST['is_active'] === '1';
$scheduleConfig = [
    'daily_time' => (string) ($_POST['daily_time'] ?? '09:00'),
    'weekly_day' => (string) ($_POST['weekly_day'] ?? '1'),
    'weekly_time' => (string) ($_POST['weekly_time'] ?? '09:00'),
    'monthly_day' => (string) ($_POST['monthly_day'] ?? '1'),
    'monthly_time' => (string) ($_POST['monthly_time'] ?? '09:00'),
    'custom_datetime' => (string) ($_POST['custom_datetime'] ?? ''),
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::validateCSRF($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            header('Location: scheduled_reports.php?success=created');
            exit;
        } catch (\Exception $e) {
            $error = $e->getMessage();
        }
...
$pageTitle = 'Create Scheduled Report - ' . brandProductName();
ob_start();
?>
        ...
        <div class="form-card">
            <form method="POST" action="" style="display: flex; flex-direction: column; gap: var(--spacing-lg);">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="schedule_name" name="schedule_name" required value="<?php echo htmlspecialchars($formScheduleName); ?>" placeholder="Weekly Sales Report">
                </div>
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label style="display: inline-flex; align-items: center; gap: var(--spacing-sm); cursor: pointer;">
                        ...
                        <span>Activate schedule immediately</span>
                    </label>
                </div>

                <div class="form-actions">
                    <a href="scheduled_reports.php" class="btn-premium-secondary">Cancel</a>
                    <button type="submit" class="btn-premium-primary">Create Schedule</button>
                </div>
            </form>
        </div>
    </div>
