...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::validateCSRF($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            $eventsModule->update($eventId, $data);
            
            header('Location: event_view.php?id=' . $eventId . '&success=updated');
            exit;
            ...
            error_log('Event update failed for event ' . $eventId . ': ' . $e->getMessage());
...
$selectedAssignedTo = array_key_exists('assigned_to', $_POST)
    ...
    : $eventsModule->resolveAssignedToForUser($event['assigned_to'] ?? null, $user);
$isAllDayEvent = (bool) (isset($_POST['is_all_day']) ? $_POST['is_all_day'] : $event['is_all_day']);
$startValue = $_POST['start_time'] ?? ($event['start_time'] ? date($isAllDayEvent ? 'Y-m-d' : 'Y-m-d\TH:i', strtotime($event['start_time'])) : '');
$endValue = $_POST['end_time'] ?? ($event['end_time'] ? date($isAllDayEvent ? 'Y-m-d' : 'Y-m-d\TH:i', strtotime($event['end_time'])) : '');
...
$pageTitle = 'Edit Event - ' . brandProductName();
ob_start();
?>

<div style="margin-bottom: var(--spacing-xl);">
    <h1 style="color: var(--midnight-black); margin-bottom: var(--spacing-sm);">Edit Event</h1>
    <p style="color: var(--charcoal-grey);">Update event details</p>
</div>
...
<div style="background: white; padding: var(--spacing-xl); border: 1px solid var(--border-color); border-radius: 8px; max-width: 800px;">
    <form method="POST" action="" style="display: flex; flex-direction: column; gap: var(--spacing-lg);">
        ...
        <input type="hidden" name="expected_lock_version" value="<?php echo (int) ($_POST['expected_lock_version'] ?? $event['lock_version'] ?? 0); ?>">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-md);">
            <div>
                ...
                <select 
                    ...
                    style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;"
                >
                    <option value="meeting" <?php echo ((isset($_POST['event_type']) ? $_POST['event_type'] : $event['event_type']) === 'meeting') ? 'selected' : ''; ?>>Meeting</option>
                    <option value="call" <?php echo ((isset($_POST['event_type']) ? $_POST['event_type'] : $event['event_type']) === 'call') ? 'selected' : ''; ?>>Call</option>
                    <option value="email" <?php echo ((isset($_POST['event_type']) ? $_POST['event_type'] : $event['event_type']) === 'email') ? 'selected' : ''; ?>>Email</option>
                    <option value="task" <?php echo ((isset($_POST['event_type']) ? $_POST['event_type'] : $event['event_type']) === 'task') ? 'selected' : ''; ?>>Task</option>
                    <option value="other" <?php echo ((isset($_POST['event_type']) ? $_POST['event_type'] : $event['event_type']) === 'other') ? 'selected' : ''; ?>>Other</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select 
                    ...
                    style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;"
                >
                    <option value="scheduled" <?php echo ((isset($_POST['status']) ? $_POST['status'] : $event['status']) === 'scheduled') ? 'selected' : ''; ?>>Scheduled</option>
                    <option value="completed" <?php echo ((isset($_POST['status']) ? $_POST['status'] : $event['status']) === 'completed') ? 'selected' : ''; ?>>Completed</option>
                    <option value="cancelled" <?php echo ((isset($_POST['status']) ? $_POST['status'] : $event['status']) === 'cancelled') ? 'selected' : ''; ?>>Cancelled</option>
                    <option value="postponed" <?php echo ((isset($_POST['status']) ? $_POST['status'] : $event['status']) === 'postponed') ? 'selected' : ''; ?>>Postponed</option>
                </select>
            </div>
        </div>
            ...
            <div>
                ...
                >
                    ...
                    <?php foreach ($contacts as $contact): ?>
                        <option value="<?php echo $contact['id']; ?>" <?php echo ((isset($_POST['contact_id']) ? $_POST['contact_id'] : $event['contact_id']) == $contact['id']) ? 'selected' : ''; ?>>
        ...
        <div>
            <label style="display: flex; align-items: center; gap: var(--spacing-sm);">
                <input 
                    ...
                    <?php echo (isset($_POST['is_all_day']) ? $_POST['is_all_day'] : $event['is_all_day']) ? 'checked' : ''; ?>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-md);">
            ...
            <div>
                ...
                >
                    ...
                    <option value="5" <?php echo ((isset($_POST['reminder_minutes']) ? $_POST['reminder_minutes'] : $event['reminder_minutes']) == '5') ? 'selected' : ''; ?>>5 minutes</option>
                    <option value="15" <?php echo ((isset($_POST['reminder_minutes']) ? $_POST['reminder_minutes'] : $event['reminder_minutes']) == '15') ? 'selected' : ''; ?>>15 minutes</option>
                    <option value="30" <?php echo ((isset($_POST['reminder_minutes']) ? $_POST['reminder_minutes'] : $event['reminder_minutes']) == '30') ? 'selected' : ''; ?>>30 minutes</option>
                    <option value="60" <?php echo ((isset($_POST['reminder_minutes']) ? $_POST['reminder_minutes'] : $event['reminder_minutes']) == '60') ? 'selected' : ''; ?>>1 hour</option>
                    <option value="1440" <?php echo ((isset($_POST['reminder_minutes']) ? $_POST['reminder_minutes'] : $event['reminder_minutes']) == '1440') ? 'selected' : ''; ?>>1 day</option>
                </select>
            </div>
        </div>
        
        <?php if ($supportsRecurrence): ?>
            <div style="background: var(--light-grey); padding: var(--spacing-md); border-radius: 4px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: var(--spacing-md);">
                    <div>
                        <label for="recurrence_pattern" style="display: block; margin-bottom: var(--spacing-xs); font-weight: 500;">Repeat</label>
                        <select id="recurrence_pattern" name="recurrence_pattern" onchange="toggleRecurrenceFields()" style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;">
                            <option value="none" <?php echo (($_POST['recurrence_pattern'] ?? $event['recurrence_pattern'] ?? 'none') === 'none') ? 'selected' : ''; ?>>None</option>
                            <option value="daily" <?php echo (($_POST['recurrence_pattern'] ?? $event['recurrence_pattern'] ?? '') === 'daily') ? 'selected' : ''; ?>>Daily</option>
                            <option value="weekly" <?php echo (($_POST['recurrence_pattern'] ?? $event['recurrence_pattern'] ?? '') === 'weekly') ? 'selected' : ''; ?>>Weekly</option>
                            <option value="monthly" <?php echo (($_POST['recurrence_pattern'] ?? $event['recurrence_pattern'] ?? '') === 'monthly') ? 'selected' : ''; ?>>Monthly</option>
                            <option value="yearly" <?php echo (($_POST['recurrence_pattern'] ?? $event['recurrence_pattern'] ?? '') === 'yearly') ? 'selected' : ''; ?>>Yearly</option>
                        </select>
                    </div>
                    <div id="recurrence_end_wrap">
                        <label for="recurrence_end_date" style="display: block; margin-bottom: var(--spacing-xs); font-weight: 500;">End Date</label>
                        <input type="date" id="recurrence_end_date" name="recurrence_end_date" value="<?php echo htmlspecialchars($_POST['recurrence_end_date'] ?? ($event['recurrence_end_date'] ? date('Y-m-d', strtotime($event['recurrence_end_date'])) : '')); ?>" style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;">
                    </div>
                    <div id="recurrence_count_wrap">
                        <label for="recurrence_count" style="display: block; margin-bottom: var(--spacing-xs); font-weight: 500;">Number of occurrences</label>
                        <input type="number" id="recurrence_count" name="recurrence_count" min="1" value="<?php echo htmlspecialchars($_POST['recurrence_count'] ?? $event['recurrence_count'] ?? ''); ?>" style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;">
                    </div>
                </div>
            </div>
        <?php else: ?>
            <div style="background: var(--light-grey); padding: var(--spacing-md); border-radius: 4px; color: var(--charcoal-grey);">
                Recurring events are not available on this database yet. You can still edit this single event normally.
            </div>
        <?php endif; ?>
        
        <div style="display: flex; gap: var(--spacing-md); justify-content: flex-end; margin-top: var(--spacing-md);">
            <a href="event_view.php?id=<?php echo $eventId; ?>" style="padding: var(--spacing-sm) var(--spacing-lg); color: var(--charcoal-grey); text-decoration: none; border: 1px solid var(--border-color); border-radius: 4px;">
                Cancel
            </a>
            <button 
                type="submit" 
...
function toggleRecurrenceFields() {
    ...
    const p = recurrence.value;
    const show = p !== 'none';
    document.getElementById('recurrence_end_wrap').style.display = show ? 'block' : 'none';
    document.getElementById('recurrence_count_wrap').style.display = show ? 'block' : 'none';
}
