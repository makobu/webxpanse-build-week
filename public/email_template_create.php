...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::validateCSRF($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            $templateId = $emailTemplatesService->create($data);
            
            header('Location: email_template_view.php?slug=' . urlencode($data['slug']) . '&success=created');
            exit;
        } catch (\Exception $e) {
            $error = $e->getMessage();
        }
...
$pageTitle = 'Create Email Template - ' . brandProductName();
ob_start();
?>

<div style="margin-bottom: var(--spacing-xl);">
    <h1 style="color: var(--midnight-black); margin-bottom: var(--spacing-sm);">Create Email Template</h1>
    <p style="color: var(--charcoal-grey);">Create a new email template</p>
</div>
...
<div style="background: white; padding: var(--spacing-xl); border: 1px solid var(--border-color); border-radius: 8px; max-width: 1000px;">
    <form method="POST" action="" style="display: flex; flex-direction: column; gap: var(--spacing-lg);">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-md);">
            ...
            <div>
                <label for="slug" style="display: block; margin-bottom: var(--spacing-xs); color: var(--midnight-black); font-weight: 500;">Slug *</label>
                <input 
                    type="text" 
                    id="slug" 
                    name="slug" 
                    ...
                    style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;"
                >
                <small style="color: var(--charcoal-grey); font-size: 12px;">Lowercase letters, numbers, and hyphens only</small>
            </div>
        </div>
        ...
        <div>
            ...
            >
                ...
                <option value="welcome" <?php echo (($_POST['category'] ?? 'general') === 'welcome') ? 'selected' : ''; ?>>Welcome</option>
                <option value="follow_up" <?php echo (($_POST['category'] ?? 'general') === 'follow_up') ? 'selected' : ''; ?>>Follow-up</option>
                <option value="notification" <?php echo (($_POST['category'] ?? 'general') === 'notification') ? 'selected' : ''; ?>>Notification</option>
                <option value="marketing" <?php echo (($_POST['category'] ?? 'general') === 'marketing') ? 'selected' : ''; ?>>Marketing</option>
                <option value="support" <?php echo (($_POST['category'] ?? 'general') === 'support') ? 'selected' : ''; ?>>Support</option>
            </select>
        </div>
        ...
        <div>
            ...
            <input 
                ...
                placeholder="e.g., Welcome to our CRM, {first_name}!"
                style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;"
            >
            <small style="color: var(--charcoal-grey); font-size: 12px;">Use {variable_name} for dynamic content</small>
        </div>
        ...
        <div>
            ...
            <textarea 
                ...
                required
                style="display: none;"
            ><?php echo htmlspecialchars($_POST['body_html'] ?? ''); ?></textarea>
        ...
        <div>
            ...
            ><?php echo htmlspecialchars($_POST['body_text'] ?? ''); ?></textarea>
        </div>
        ...
        <div>
            ...
            <input 
                ...
                id="variables" 
                name="variables" 
                value="<?php echo htmlspecialchars($_POST['variables'] ?? ''); ?>"
                placeholder="e.g., first_name, last_name, email, company"
                style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--border-color); border-radius: 4px;"
            >
            <small style="color: var(--charcoal-grey); font-size: 12px;">List all variables used in the template (without curly braces)</small>
        </div>
        ...
        <div style="display: flex; gap: var(--spacing-md); justify-content: flex-end; margin-top: var(--spacing-md);">
            <a href="email_templates.php" style="padding: var(--spacing-sm) var(--spacing-lg); color: var(--charcoal-grey); text-decoration: none; border: 1px solid var(--border-color); border-radius: 4px;">
                Cancel
            </a>
            <button 
                type="submit" 
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize Quill editor for email template
    const editorContainer = document.getElementById('body_html_editor');
    const hiddenTextarea = document.getElementById('body_html');
    
    if (editorContainer && hiddenTextarea) {
        const quill = new Quill('#body_html_editor', {
            theme: 'snow',
            modules: {
                toolbar: [
                    [{ 'header': [1, 2, 3, false] }],
