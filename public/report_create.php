...
$templateName = trim((string) ($_GET['template'] ?? ''));
$selectedTemplate = null;
if ($templateName !== '') {
    foreach ($reportsModule->getTemplates() as $template) {
...
$templateQueryConfig = (array) ($selectedTemplate['query_config'] ?? []);
$templateFilters = (array) ($selectedTemplate['filters'] ?? []);

$formType = (string) ($_POST['report_type'] ?? ($selectedTemplate['report_type'] ?? 'contacts'));
$submittedQueryConfig = [
    'fields' => !empty($_POST['fields']) ? array_filter(array_map('trim', explode(',', (string) $_POST['fields']))) : ($templateQueryConfig['fields'] ?? []),
    'order_by' => (string) ($_POST['order_by'] ?? ($templateQueryConfig['order_by'] ?? '')),
    'order_dir' => (string) ($_POST['order_dir'] ?? ($templateQueryConfig['order_dir'] ?? 'DESC')),
    'limit' => $_POST['limit'] ?? ($templateQueryConfig['limit'] ?? null),
];
$submittedFilters = [
    'date_from' => $_POST['date_from'] ?? ($templateFilters['date_from'] ?? null),
    'date_to' => $_POST['date_to'] ?? ($templateFilters['date_to'] ?? null),
    'stage' => $_POST['stage'] ?? ($templateFilters['stage'] ?? null),
    'lead_source' => $_POST['lead_source'] ?? ($templateFilters['lead_source'] ?? null),
    'status' => $_POST['status'] ?? ($templateFilters['status'] ?? null),
    'priority' => $_POST['priority'] ?? ($templateFilters['priority'] ?? null),
    'event_type' => $_POST['event_type'] ?? ($templateFilters['event_type'] ?? null),
];
$preparedState = $reportsModule->prepareConfiguration($formType, $submittedQueryConfig, $submittedFilters);

$formName = (string) ($_POST['name'] ?? ($selectedTemplate['name'] ?? ''));
$formDescription = (string) ($_POST['description'] ?? ($selectedTemplate['description'] ?? ''));
$formIsPublic = isset($_POST['is_public']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::validateCSRF($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            header('Location: report_view.php?id=' . $reportId . '&success=created');
            exit;
        } catch (\Exception $e) {
            $error = $e->getMessage();
        }
...
$pageTitle = 'Create Report - ' . brandProductName();
ob_start();
?>
...
<div class="page-premium">
    <div class="container">
        <div class="page-header">
            <div>
                <h1>Create Report</h1>
                <p>Build a custom report from your business data</p>
            </div>
        ...
        <div class="form-card">
            <form method="POST" action="" style="display: flex; flex-direction: column; gap: var(--spacing-lg);" id="report-builder-form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" required value="<?php echo htmlspecialchars($formName); ?>" placeholder="Monthly Sales Report">
                </div>

                <div class="form-group">
                    <label for="description">Description</label>
                    <textarea id="description" name="description" rows="3" placeholder="Describe what this report shows..."><?php echo htmlspecialchars($formDescription); ?></textarea>
                </div>
                ...
                <div class="content-card" style="margin-bottom: 0;">
                    <h2 style="font-size: 1.125rem; margin: 0 0 1rem 0;">Filters</h2>
                ...
                <div class="form-actions">
                    <a href="reports.php" class="btn-premium-secondary">Cancel</a>
                    <button type="submit" class="btn-premium-primary">Create Report</button>
                </div>
            </form>
        </div>
    </div>
