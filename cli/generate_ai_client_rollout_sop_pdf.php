...
$source = __DIR__ . '/../docs/ai-client-rollout-sop.md';
$output = __DIR__ . '/../docs/ai-client-rollout-sop.pdf';
...
function renderMarkdownToHtml(string $markdown): string
{
    ...
    foreach ($lines as $line) {
        ...
        if (str_starts_with($trimmed, '- ')) {
            ...
            $html[] = '<li>' . preg_replace('/`([^`]+)`/', '<code>$1</code>', htmlspecialchars(substr($trimmed, 2), ENT_QUOTES, 'UTF-8')) . '</li>';
            continue;
        }
...
$pdf->SetCreator('CRM');
$pdf->SetAuthor('Codex');
$pdf->SetTitle('AI Client Rollout SOP');
$pdf->SetSubject('Initial best settings and client rollout SOP');
