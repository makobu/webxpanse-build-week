...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || (int) ($_SERVER['CONTENT_LENGTH'] ?? 0) > 16384) {
...
try {
    echo (new \CRM\Services\VoiceWebhookService())->handleConsent(
        (string) ($_GET['token'] ?? ''),
        $_POST,
        (string) ($_SERVER['REMOTE_ADDR'] ?? '')
    );
} catch (Throwable $e) {
    error_log('Voice consent callback error: ' . $e->getMessage());
    echo '<?xml version="1.0" encoding="UTF-8"?><Response><Reject /></Response>';
}
