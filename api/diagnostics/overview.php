<?php

require_once __DIR__ . '/../../vendor/autoload.php';

$envFile = __DIR__ . '/../../.env';
if (file_exists($envFile)) {
    $lines = file($envFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
    foreach ($lines as $line) {
        if (strpos(trim($line), '#') === 0 || strpos($line, '=') === false) {
            continue;
        }
        [$key, $value] = explode('=', $line, 2);
        $_ENV[trim($key)] = trim($value);
    }
}

require_once __DIR__ . '/../../config/constants.php';

use CRM\Auth;
use CRM\Authorization;
use CRM\Database;
use CRM\Services\AIAutomationDiagnosticsService;
use CRM\Session;

Database::init(require __DIR__ . '/../../config/database.php');
Session::start();

header('Content-Type: application/json');

if (!Auth::check()) {
    http_response_code(401);
    echo json_encode(['success' => false, 'error' => 'Unauthorized']);
    exit;
}
Authorization::requirePermission('ai.operations.manage', true);

$service = new AIAutomationDiagnosticsService();
$filters = [
    'source' => trim((string) ($_GET['source'] ?? '')),
    'decision' => trim((string) ($_GET['decision'] ?? '')),
    'user_id' => (int) ($_GET['user_id'] ?? 0),
    'deal_id' => (int) ($_GET['deal_id'] ?? 0),
    'invoice_id' => (int) ($_GET['invoice_id'] ?? 0),
    'contact_id' => (int) ($_GET['contact_id'] ?? 0),
    'task_id' => (int) ($_GET['task_id'] ?? 0),
    'date_from' => trim((string) ($_GET['date_from'] ?? '')),
    'date_to' => trim((string) ($_GET['date_to'] ?? '')),
    'reason_bucket' => trim((string) ($_GET['reason_bucket'] ?? '')),
    'mode' => trim((string) ($_GET['mode'] ?? '')),
];

echo json_encode([
    'success' => true,
    'summary' => $service->getSummary($filters),
    'top_blockers' => $service->getTopBlockers($filters),
    'context_health' => $service->getContextHealth($filters),
    'recent_degraded' => array_values(array_filter(
        $service->getRecentEvents($filters + ['limit' => 10]),
        static fn (array $event): bool => in_array((string) ($event['decision'] ?? ''), ['blocked', 'approval_required', 'retry_pending', 'failed'], true)
            || (string) ($event['source'] ?? '') === 'capability'
    )),
]);
