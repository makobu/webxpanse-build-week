...
$bundleKey = trim((string) ($input['bundle_key'] ?? $input['activation_bundle_key'] ?? ''));
if ($bundleKey === '') {
    http_response_code(422);
    echo json_encode(['success' => false, 'error' => 'Missing activation bundle']);
    exit;
}
...
try {
    (new WorkspaceMarketplaceActivationBundleEventService())->recordEvent(
        ...
        (int) ($user['id'] ?? 0),
        $bundleKey,
        (string) ($input['event_type'] ?? 'cta_clicked'),
        [
            'surface' => 'marketplace',
            'metadata' => [
                ...
                'next_action_skill_key' => (string) ($input['next_action_skill_key'] ?? $input['skill_key'] ?? ''),
            ],
        ]
    );
    ...
    error_log('Marketplace activation bundle event API error: ' . $e->getMessage());
