...
$goal = trim((string) ($input['goal'] ?? 'draft'));
...
if ($action === 'revise_reply') {
    $service = new \CRM\Services\EmailAssistantApplicationService();
    $result = $service->previewCustomerThreadCommercialReply($communicationId, $userId, [
        'goal' => 'draft',
        'revise_document' => true,
        'apply_document_revision' => false,
    ]);
} else {
    $result = $handler->handleCustomerThreadDraft($communicationId, $userId, ['goal' => $goal]);
}
$draft = $result['draft'] ?? [];

echo json_encode([
    'success' => true,
    'mode' => $result['mode'] ?? 'customer_thread',
