...
try {
    ...
    if ((int) ($oauthContext['user_id'] ?? 0) !== $currentUserId) {
        throw new \RuntimeException('Google Calendar authorization no longer matches the signed-in user.');
    }
    ...
    if ($currentWorkspaceId > 0 && (int) ($oauthContext['workspace_id'] ?? 0) > 0 && $currentWorkspaceId !== (int) $oauthContext['workspace_id']) {
        throw new \RuntimeException('Google Calendar authorization no longer matches the active workspace.');
    }
    ...
    $grantType = GoogleOAuthScopeCatalog::normalizeGrantType((string) $oauthContext['grant_type']);
    ...
    if (!GoogleOAuthScopeCatalog::hasRequiredScopes($grantedScopes, $requiredScopes)) {
        if (Database::tableExists('oauth_connection_audit_log')) {
            Database::execute(
                ...
                ) VALUES (?, ?, 'google', 'calendar', ?, ?, 'failed', 'scope_mismatch', ?, ?, ?)",
                [
                    $workspaceId,
                    (int) $oauthContext['user_id'],
                    $grantType,
                    (string) ($oauthContext['oauth_client_key'] ?? $catalog['client_key']),
                    'Google did not return every scope required for ' . $catalog['label'] . '.',
                    json_encode(is_array($oauthContext['requested_scopes'] ?? null) ? $oauthContext['requested_scopes'] : $requiredScopes, JSON_UNESCAPED_SLASHES),
                    json_encode($grantedScopes, JSON_UNESCAPED_SLASHES),
                ]
            );
        }
        throw new \RuntimeException($catalog['label'] . ' is missing required Google permissions. Please reconnect and approve the requested access.');
    }
    ...
    if ($storedAccessToken === null) {
        throw new \RuntimeException('Google Calendar OAuth did not return an access token.');
    }
    $providerEmail = $service->fetchAccountEmail((string) $accessToken);

    $existing = Database::queryOne(
        "SELECT id, refresh_token, sync_enabled FROM calendar_integrations WHERE user_id = ? AND workspace_id = ? AND provider = 'google'",
        [(int) $oauthContext['user_id'], $workspaceId]
    );
    ...
    if ($existing) {
        ...
        Database::execute(
            "UPDATE calendar_integrations
             SET oauth_grant_type = ?,
                 ...
                 sync_enabled = ?,
                 sync_direction = CASE WHEN ? = 'calendar_import' THEN 'to_crm' ELSE sync_direction END,
                 updated_at = NOW()
        ...
        Database::execute(
            "INSERT INTO calendar_integrations (
                ...
                provider_account_email, access_token, refresh_token, token_expires_at, granted_scopes_json,
                required_scopes_json, scope_status, reconnect_required, token_encrypted,
                last_scope_verified_at, calendar_id, availability_enabled, sync_enabled, sync_direction
             ) VALUES (?, ?, 'google', ?, ?, ?, ?, ?, ?, ?, ?, 'verified', 0, 1, NOW(), 'primary', 1, ?, 'to_crm')",
            [
                $workspaceId,
                (int) $oauthContext['user_id'],
                $grantType,
    ...
    if (Database::tableExists('oauth_connection_audit_log')) {
        Database::execute(
            ...
            ) VALUES (?, ?, 'google', 'calendar', ?, ?, 'success', ?, ?, ?, ?, 'calendar_integrations', ?)",
            [
                $workspaceId,
                (int) $oauthContext['user_id'],
                $grantType,
    ...
    $target = $appendQuery($target, ['success' => 'google_connected']);
