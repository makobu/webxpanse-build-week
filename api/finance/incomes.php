...
$service = new FinanceIncomeService();

try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        Authorization::requirePermission('finance.view', true);
        echo json_encode([
            'income_categories' => $service->listCategories($workspaceId),
            'income_entries' => $service->listEntries($workspaceId, $_GET, 100, 0),
        ]);
        exit;
    }
    ...
    if ((string) ($input['action'] ?? '') === 'delete_income') {
        $service->deleteEntry($workspaceId, (int) ($input['id'] ?? 0));
        echo json_encode(['success' => true]);
        exit;
    }
    if ((string) ($input['action'] ?? '') === 'save_income_category') {
        $id = $service->saveCategory($workspaceId, (string) ($input['name'] ?? ''), (string) ($input['income_type'] ?? 'sale'), $userId);
        echo json_encode(['success' => true, 'id' => $id]);
        exit;
    }

    $id = $service->saveEntry($workspaceId, $input, $userId);
    echo json_encode(['success' => true, 'id' => $id]);
} catch (\Throwable $e) {
