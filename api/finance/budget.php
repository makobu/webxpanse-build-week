...
$service = new FounderFinanceService();

try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        Authorization::requirePermission('finance.view', true);
        echo json_encode($service->getFounderBudget($workspaceId, $userId, $_GET['date_from'] ?? null, $_GET['date_to'] ?? null));
        exit;
    }
    ...
    $id = $service->saveFounderBudget($workspaceId, $userId, $input);
